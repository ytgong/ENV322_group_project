...
linregs.all<-c(-3.182e-3,-3.034e-4,-3.510e-8,-4.608e-5,0.0060945,-0.0221976,
                -0.0001424,2.515e-4,-1.881e-3,-3.747e-4,3.953e-4,-6.377e-4,
                0.001523,1.659e-3,-0.0003337,-7.366e-6,-2.001e-4,1.792e-4,
                -7.248e-5,-9.185e-5,9.849e-5)

#linregs.abs<-abs(linregs)
linregs.abs.sig<-abs(linregs.sig)
linregs.abs.all<-abs(linregs.all)

hyp1_analysis<-tibble(linregs.all, linregs.abs.all, sites = linreg.sites)%>%
  left_join(RBI_ind, by = "sites")

hyp1_sig<-hyp1_analysis%>%
  filter(sites %in% c("BLUE", "BOW","CHAS", "CRANE","DRAIN","FELL",
                "HOL","HOM","HUN","ICHE","MAD","MAIN","MAN","RAIN",
                "SANTA","THREE","TURK","WAC"))

hyp1_nocanals<-hyp1_sig%>%
  filter(sites %in% c("BLUE","BOW","CAL","CHAS","CRANE","FAN",
                "HOL","HOM","HUN","ICHE","MAD","MAN","RAIN",
                "SANTA","TURK","TURN","WAC"))

write_csv(hyp1_sig, "./RBI_CHEMO_data.csv")
write_csv(hyp1_nocanals, "./RBI_CHEMO_nocanals.csv")
```
